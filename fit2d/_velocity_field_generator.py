...
def create_2d_velocity_field(
    ...
    # to do: vecctorize to produce array of (y, x, v_los) and write func
    # to convert coord-value array into 2d array
...
def _convert_galaxy_to_observer_coords(ring_model, r, theta, kpc_per_pixel):
    ...
    x_kpc = -r * (
        -np.cos(pos_ang) * np.cos(theta) + np.sin(pos_ang) * np.sin(theta) * np.cos(inc)
    )
    y_kpc = r * (
        np.sin(pos_ang) * np.cos(theta) + np.cos(pos_ang) * np.sin(theta) * np.cos(inc)
    )
