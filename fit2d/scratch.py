...
if not dynamic:
    sampler = dynesty.NestedSampler(
        log_likelihood, prior_transform, ndim, bound=method, nlive=nlive
    )
    sampler.run_nested(dlogz=tol, print_progress=True)
else:
    sampler = dynesty.DynamicNestedSampler(log_likelihood, prior_transform, ndim)
    sampler.run_nested(
        maxiter=10000, print_progress=True, use_stop=False, wt_kwargs={"pfrac": 1.0}
    )
...
)  # get nested sampling samples and weights
new_samples = dynesty.utils.resample_equal(
    samples, weights
)  # resample to get a smaller number of equally weighted samples
samples = new_samples[
    np.random.choice(len(new_samples), nsamples)
]  # now we have "nsamples" equally-weighted "samples"
